...
void display_toast(char *string) {
  ...
  *user_longitude_tuple;
  ...
  if ((display_message_tuple = dict_find(iterator, KEY_DISPLAY_MESSAGE))) {
    if(display_message_tuple->value->cstring) {
      display_toast(display_message_tuple->value->cstring);
    }
  }
...
static void bluetooth_callback(bool connected) {
  if(connected) {
    display_toast("Bluetooth reconnected");
  } else {
    display_toast("Bluetooth disconnected");
  }
}
